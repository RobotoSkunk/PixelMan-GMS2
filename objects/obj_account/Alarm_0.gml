/// @description Leer archivos para guardar nivel
var txt = "";
var content = "";
file_length = 0;
file_load = 0;
...
if(fileName[0] != ""){
	...
	if(!ini_section_exists("Data")){
		...
		var pos = 0;
		while(file_exists(directory[0] + string(pos) + ".pml")){
			pos++;
		}
	...
	alarm[0] = file_length/2;
} else {
	file_find_close();
	
	/*var file = file_text_open_write("Result.pmx");
	...
	post = http_post_string("http://pixelmanserver.000webhostapp.com/user_save_data.php", "id=" + string(ID) + "&final=" + final_text);
	
	state = text[79];
	
}

/*var file = file_text_open_read(directory[0] +  fileName[0]);

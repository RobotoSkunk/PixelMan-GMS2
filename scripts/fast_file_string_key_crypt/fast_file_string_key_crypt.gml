/// @description fast_file_string_key_crypt(input_file, output_file, is_encrypted, encryption_key_string);
/// @function fast_file_string_key_crypt
/// @param input_file
...
fast_text_key_crypt("game_level_01.ini", "game_level_01.lev", true, "SomeSecretKey");
* Decrypting to a different file
fast_text_key_crypt("game_level_01.lev", "game_level_01.ini", false, "SomeSecretKey");

* Encrypting to the same file:
fast_text_key_crypt("user_progress.dat", "user_progress.dat", true, "SomeSecretKey");
* Decrypting to the same file:
fast_text_key_crypt("user_progress.dat", "user_progress.dat", false, "SomeSecretKey");

*********************************************************************
*/

var data, filename_in, pos, len, xor_shift, bit_shift, enc_key, key_hash, key_hash2, key_pos, key_len;
...
while(pos != len) // Processing each byte in the buffer step by step until the end of the buffer
{
	...
	bit_shift -= (key_arr[(key_len-1)-key_pos] mod 2);
...
var rtrn = buffer_base64_encode(file_buffer, 0, buffer_get_size(file_buffer));
//var rtrn = buffer_read(file_buffer, buffer_u8);
buffer_delete(file_buffer); // Removing buffer from memory

//show_debug_message(base64_decode(rtrn));
